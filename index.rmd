...
summary(model)
```
```{r keras-test-2, echo=FALSE}

increment <- 1        # number of epochs to train per run
current.epoch <- -1   # index tracking total number of epochs trained
...
ui.training <-
  fluidPage(
    fluidRow(
      column(4, align="center",
        actionButton("my.button", label="Train another epoch"),
        br(),
        h1(textOutput("print.counter"))
      ),
...
server.training <- function(input, output){
  ...
  output$print.counter <- renderText({
    input$my.button

    # HACK: Oox of print.counter and plot.history is not defined.
    #       Currently, print.counter seems to evaluate current.epoch before it's incremented in plot.history.
    #       Define a proper order of execution once a suitable mechanism is discovered.
  ...
  output$plot.history <- renderPlot({
    ...
    withProgress(message = 'Making plot', value = 0, {
      ...
      {
        training.history <- model %>% fit(x.train, y.train,
                                         initial_epoch = current.epoch,
                                         epochs = current.epoch + increment)
        print(paste0("train_hist =", training.history$metrics))

        accuracy <<- c(accuracy, training.history$metrics$accuracy)

        incProgress(1/increment, detail = paste("Doing part", ii))
      }
    })
    plt <- plt + geom_point(aes(x = 1:current.epoch, y = accuracy))

    # increment epoch counter
    current.epoch <<- current.epoch+increment
    return(plt)
 })
}
